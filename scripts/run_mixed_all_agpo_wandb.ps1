...
$agpoConfig = "examples\train_lora\qwen3_0_6b_mixed_all_agpo_wandb.yaml"
$rewardStdoutLog = Join-Path $repoRoot "saves\qwen3-0.6b\lora\hybrid_reward_server.out.log"
$rewardStderrLog = Join-Path $repoRoot "saves\qwen3-0.6b\lora\hybrid_reward_server.err.log"

Set-Location $repoRoot
New-Item -ItemType Directory -Force -Path (Split-Path -Parent $rewardStdoutLog) | Out-Null

$env:WANDB_PROJECT = "agpo-mixed-all"
$env:WANDB_RUN_GROUP = "qwen3-0.6b-mixed-all"
...
print(json.dumps({
    ...
    'code_dataset_dir': str(config.get('code_reward_dataset_dir', 'data/code_contests/hf_dataset')),
...
$env:HYBRID_ANSWER_DATA = $rewardConfig.answer_data
$env:CODE_REWARD_DATA = $rewardConfig.code_dataset_dir
...
$server = Start-Process `
    -FilePath $python `
    -ArgumentList @(
        "scripts\hybrid_reward_server.py",
        "--answer-data", $rewardConfig.answer_data,
        "--code-dataset-dir", $rewardConfig.code_dataset_dir,
